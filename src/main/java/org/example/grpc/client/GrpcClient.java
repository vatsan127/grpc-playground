...
public class GrpcClient {
    ...
    public static void main(String[] args) {
        ManagedChannel builder = ManagedChannelBuilder.forAddress("localhost", port)
                /*use plain text when ssl certificate is not present*/
                .usePlaintext()
                .build();
        var stub = org.example.grpc.collection.BankServiceGrpc.newBlockingStub(builder);
        BankServiceOuterClass.BankRequest request = BankServiceOuterClass.BankRequest.newBuilder().setAccountNumber(127).build();
